...
public class BCFC : MonoBehaviour
{
    ...
    public class LAYER
    {
        ...
        public GameObject newImageObjectReferece;
        ...
        public void SetTexture(Texture texture, bool ifMovieThenLoop = true)
        {
            ...
            if (texture!=null)
            {
                ...
                activeImage.color = FunctionsVN.SetAlpha(activeImage.color, 1f);
                /*
                MovieTexture mov = texture as MovieTexture;
                if (mov != null)
                {
        ...
        IEnumerator Transitioning(Texture texture, float speed, bool smooth, bool ifMovieThenLoop)
        {
            if (texture != null)
            {
                ...
                if (activeImage == null || activeImage.texture != texture)
                {
                    CreateNewActiveImage();
                    activeImage.texture = texture;
                    activeImage.color = FunctionsVN.SetAlpha(activeImage.color, 0f);
                    /*
                    MovieTexture mov = texture as MovieTexture;
                    if (mov != null)
                    {
            ...
            while (FunctionsVN.TransitionRawImages(ref activeImage, ref allImages, speed,smooth))
            {
        ...
        {
            GameObject go = Instantiate(newImageObjectReferece, root.transform) as GameObject;
            go.SetActive(true);
